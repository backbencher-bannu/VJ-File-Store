...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('💝 website', url='https://ynvmovies.xyz/')
            ],[
            InlineKeyboardButton('🔍 request ɢʀᴏᴜᴘ', url='https://t.me/ynvmoviesrequest1'),
            InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ', url='https://t.me/ynvmoviesupdates')
            ],[
...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            title = '@ynvmovies  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
